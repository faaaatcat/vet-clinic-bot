...
def _add_test_data(cur):
    ...
    # === Услуги ===
    # Сначала проверяем, есть ли уже услуги в базе
    ...
    if services_count == 0:
        ...
        for service in services:
            cur.execute("INSERT INTO services (name, duration, price) VALUES (?, ?, ?)", service)

    # === Связь врачей и услуг ===
    # Проверяем, есть ли уже связи
    cur.execute("SELECT COUNT(*) FROM doctor_services")
    doctor_services_count = cur.fetchone()[0]

    # Добавляем связи только если их нет
    if doctor_services_count == 0:
    ...
    cur.execute("SELECT COUNT(*) FROM users WHERE telegram_id LIKE '1000000%'")
    test_users_count = cur.fetchone()[0]

    # Добавляем тестовых пользователей только если их нет
    if test_users_count == 0:
        ...
        for user in users:
            cur.execute(
                "INSERT INTO users (telegram_id, phone, full_name) VALUES (?, ?, ?)",
                user
            )

    # === Питомцы ===
    # Проверяем, есть ли уже тестовые питомцы
    cur.execute("SELECT COUNT(*) FROM pets")
    pets_count = cur.fetchone()[0]

    # Добавляем тестовых питомцев только если их нет
    if pets_count == 0:
    ...
    print("✅ Тестовые данные проверены/добавлены (без дублирования)")
